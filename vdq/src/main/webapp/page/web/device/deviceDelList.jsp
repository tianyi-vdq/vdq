				...
				<table class="yw-cm-table yw-center yw-bg-hover" id="deviceList">
					...
					<c:forEach var="item" items="${Devicelist}">
						<tr>
							...
							<td onclick="window.location.href='<%=basePath%>device/deviceInfo.do?pointId=${item.id}'">已移除</td>
							...
							<td onclick="window.location.href='<%=basePath%>device/deviceInfo.do?pointId=${item.id}'">${item.delTimes}</td>
							<td onclick="window.location.href='<%=basePath%>device/deviceInfo.do?pointId=${item.id}'">${item.description}</td>
							<td>
								<a href="javascript:void(0);" onclick="stopOrStartDevice(${item.id},3);" style="color:blue">恢复设备</a>  
							</td>    
						</tr>
					</c:forEach>
				</table> 
